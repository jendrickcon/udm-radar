...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['action'] ?? '') === 'submit_batch') {
    if (!isset($_POST['csrf_token']) || !hash_equals($_SESSION['csrf_token'] ?? '', $_POST['csrf_token'])) {
        ...
        if (!$subjId || !$section || !in_array($termType, ['prelim', 'midterm', 'prefinal', 'final_grade'])) {
            ...
            foreach ($newGrades as $studentId => $gradeVal) {
                ...
                if ($gradeVal !== '') {
                    ...
                    if ($reason === '') {
                        $error = "A reason is required for every modified grade.";
                        break;
                    }
                    $payload[] = [
            ...
            if (!$error) {
                if (empty($payload)) {
                    $error = 'No grades were entered to submit.';
                } else {
                    try {
...
<div class="main-content">
    <div class="header" style="margin-bottom: 24px;">
        <div>
            <h1>Encode Grades</h1>
            <p>Select your assigned class and term to submit grade proposals to Administration.</p>
        </div>
    </div>
        ...
        <form method="GET" action="grades.php" style="display:flex; gap:16px; align-items:center; flex-wrap:wrap;">
            ...
            <div style="display:flex; flex-direction:column; gap:4px; flex-grow:1; max-width: 200px;">
                ...
                <select id="termSelect" style="padding: 10px; border-radius: 8px; border: 1px solid var(--border-color); background-color: var(--bg-color); color: var(--text-dark); font-family: inherit;">
                    ...
                    <option value="final_grade" <?= $selectedTerm === 'final_grade' ? 'selected' : '' ?>>Final Grade</option>
                </select>
            </div>
        </form>
    </div>

    <!-- Messages Container (For "Loading", "Empty", or "Pending" states) -->
    <div id="message-container" style="display: none; margin-bottom: 24px;"></div>

    <!-- Main Table Card (Permanently structurally embedded, hidden when empty) -->
    <div id="roster-card" class="card animate-fade-up" style="display: none; padding: 0; overflow: hidden;">
        ...
        <form method="POST" action="grades.php" style="margin: 0;">
                ...
                <table style="width: 100%; border-collapse: collapse; font-size: 0.9rem;">
                    <thead>
                        <tr style="background: var(--table-header-bg);">
                            ...
                            <th style="padding: 14px 24px; text-align: left; color: var(--text-dark);">Reason for Change</th>
                        </tr>
                    </thead>
            ...
            <div style="padding: 20px 24px; background: var(--bg-color); text-align: right; border-top: 1px solid var(--border-color);">
                <button type="submit" style="background:var(--accent-blue); color:white; border:none; padding:12px 24px; border-radius:8px; font-weight:600; cursor:pointer; font-size: 0.95rem; transition: opacity 0.2s;" onmouseover="this.style.opacity=0.9" onmouseout="this.style.opacity=1">
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function loadRoster(userInitiated = false) {
        ...
        // If table is currently showing, don't destroy it. Just dim the rows to indicate loading.
        ...
        fetch(`grades.php?action=fetch_roster&class=${encodeURIComponent(classSelect.value)}&term=${encodeURIComponent(termSelect.value)}`)
            ...
            .catch(err => {
                tbody.style.opacity = '1';
                tbody.style.pointerEvents = 'auto';
                showMessage(`<div class="card animate-fade-up" style="text-align:center; padding: 40px; color: var(--risk-high);">Error loading roster. Please try again.</div>`);
            });
    }

    function renderTable(data) {
        if (data.hasPendingBatch) {
            showMessage(`<div class="animate-fade-up" style="background:rgba(217, 119, 6, 0.1); border:1px solid var(--risk-mod); border-radius:8px; padding:16px;"><h3 style="color:var(--risk-mod); font-size:1.05rem; margin-bottom:4px;">Batch Pending Approval</h3><p style="color:var(--text-dark); font-size:0.9rem;">You have already submitted a grade batch for this term. Please wait for Administration to review and approve it before submitting another.</p></div>`);
            return;
        }
        ...
        // We have data: hide messages, reveal the card
        msgContainer.style.display = 'none';
        rosterCard.style.display = 'block';

        // Update the card headers
        ...
        // Update hidden form inputs for backend POST
        ...
        // Build and inject ONLY the rows
        let rowsHtml = '';
        data.students.forEach(s => {
            let currentVal = s[data.term];
            currentVal = currentVal !== null ? parseFloat(currentVal).toFixed(2) : '—';
            ...
            rowsHtml += `
                <tr style="border-bottom: 1px solid var(--border-color);">
                    ...
                    <td style="padding: 12px; text-align: center; font-weight: 600; color: var(--text-gray);">${currentVal}</td>
                    <td style="padding: 12px;">
                        <input type="text" name="grades[${s.student_id}]" placeholder="e.g. 95" 
                               style="width: 100%; padding: 8px; border: 1px solid var(--border-color); border-radius: 6px; background: var(--bg-color); color: var(--text-dark); text-align: center;"
                               oninput="document.getElementById('reason_${s.student_id}').required = (this.value.trim() !== '');">
                    </td>
                    <td style="padding: 12px 24px;">
                        <input type="text" id="reason_${s.student_id}" name="reasons[${s.student_id}]" placeholder="Required if grade is entered..." 
                               style="width: 100%; padding: 8px; border: 1px solid var(--border-color); border-radius: 6px; background: var(--bg-color); color: var(--text-dark);">
                    </td>
                </tr>
            `;
        });
    ...
    // Pass 'true' so smooth-scroll only happens when the user manually clicks the dropdown
    classSelect.addEventListener('change', () => loadRoster(true));
    termSelect.addEventListener('change', () => loadRoster(true));

    // Run on initial load (allows redirect links from feedback.php to auto-load the table)
    loadRoster(false);
});
</script>
