...
<div class="main-content">
    ...
    <div class="card">
        ...
        <form method="POST" action="faculty.php" style="display:grid; grid-template-columns: repeat(3, 1fr); gap:14px;">
            ...
            <button type="submit" style="padding:10px; background:var(--accent-blue); color:white; border:none; border-radius:8px; font-weight:600; cursor:pointer; align-self:start;">Add Faculty</button>
        </form>
    </div>
        ...
        <table style="width:100%; border-collapse: collapse;">
            <thead>
                <tr style="background: var(--table-header-bg); border-bottom: 1px solid var(--border-color);">
                    ...
                    <th style="padding: 8px; text-align: left; color: var(--text-dark);">Class Avg</th>
    ...
    <div class="modal-box" style="max-width:680px;">
        ...
        <form method="POST" action="faculty.php" id="edit-form">
            ...
            <div style="display:grid; grid-template-columns:2fr 1fr; gap:10px; margin-bottom:18px;">
                <select name="new_subject_id" class="form-input">
                    <option value="">— Select subject —</option>
                    <?php foreach ($allSubjects as $subj): ?>
                        <option value="<?= $subj['id'] ?>"><?= htmlspecialchars($subj['code'] . ' — ' . $subj['title']) ?></option>
                    <?php endforeach; ?>
                </select>
                <input type="text" name="new_section" placeholder="Section (e.g. IT-33)" class="form-input">
            </div>
    ...
    if (!d.loads.length) {
        ...
        body.innerHTML = d.loads.map(l => {
            ...
            return `<tr style="border-bottom: 1px solid var(--border-color);">
                <td style="font-weight:600; color:var(--text-dark); padding: 12px;">${l.section}</td>
                <td title="${l.title}" style="color:var(--text-gray); padding: 12px;">${l.code}</td>
                <td style="font-weight:600; color:var(--text-dark); padding: 12px;">${avg}</td>
                <td style="color:${atRiskColor}; font-weight:600; padding: 12px;">${l.atRisk} / ${l.students}</td>
            </tr>`;
        }).join('');
    }
    ...
    if (!d.loads.length) {
        ...
        list.innerHTML = d.loads.map(l => `
            <label style="display:flex; align-items:center; gap:8px; padding:8px 0; font-size:0.88rem; border-bottom:1px solid var(--border-color);">
                ...
                <span style="margin-left:auto; font-weight:600; color:var(--text-dark);">${l.section}</span>
                <span style="color:var(--text-gray);" title="${l.title}">${l.code}</span>
            </label>
        `).join('');
    }
